...
import { Users, Search, Plus, ArrowUpRight, ArrowDownRight, FileText, X, CheckCircle2 } from 'lucide-react';
import { Customer, CustomerTransaction } from '../types';

export default function Customers() {
  const { customers, setCustomers, settings, currentUser } = useAppContext();
  ...
  const hasAccess = currentUser?.role === 'مدير' || settings.permissions[currentUser?.role || '']?.customers;
  ...
  if (!hasAccess) {
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] animate-in fade-in zoom-in duration-150">
        ...
        <p className="text-slate-600 text-center max-w-md">عذراً، لا تملك الصلاحيات الكافية للوصول إلى إدارة العملاء.</p>
      </div>
    );
  }
  ...
  const handleAddPayment = (e: React.FormEvent) => {
    ...
    const newTx: CustomerTransaction = {
      ...
      amount,
      type: 'دائن', // Payment reduces debt
      description: paymentNotes || 'دفعة سداد نقدية',
    };

    const updatedCustomer = {
      ...selectedCustomer,
      balance: selectedCustomer.balance - amount,
      totalPaid: selectedCustomer.totalPaid + amount,
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-2">
            متابعة أرصدة العملاء، وتسجيل دفعات السداد، واستخراج كشوفات الحساب.
          </p>
        </div>
      </div>
            ...
            <table className="w-full text-right text-sm">
              <thead className="bg-slate-50 text-slate-600 font-semibold border-b border-slate-100 ">
                <tr>
                  ...
                  <th className="px-6 py-4">إجمالي الفواتير</th>
                  <th className="px-6 py-4">إجمالي المدفوع</th>
                  <th className="px-6 py-4 text-rose-600">الرصيد المتبقي (عليه)</th>
                ...
                {filteredCustomers.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-600">
                      لا يوجد عملاء متاحين. يتم إضافة العملاء تلقائياً عند إنشاء طلب آجل.
                    </td>
                  </tr>
                ) : (
                  filteredCustomers.map(customer => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">{customer.totalInvoiced.toLocaleString()} د.ل</td>
                      <td className="px-6 py-4 text-emerald-600">{customer.totalPaid.toLocaleString()} د.ل</td>
                      <td className="px-6 py-4 font-bold text-rose-600">{customer.balance.toLocaleString()} د.ل</td>
                      <td className="px-6 py-4 text-xs text-slate-600">
                        {customer.lastPaymentDate ? new Date(customer.lastPaymentDate).toLocaleDateString('ar-LY') : 'لا يوجد'}
                      </td>
                    </tr>
                  ))
                )}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white border border-slate-200/80 rounded-xl p-6 shadow-sm md:col-span-2 flex justify-between items-center">
              <div>
                <h2 className="text-xl font-bold text-slate-900 mb-1">{selectedCustomer.name}</h2>
                <p className="text-sm text-slate-600">كشف الحساب وتفاصيل المعاملات المالية</p>
              </div>
              <button 
            ...
            <div className="bg-rose-50 border border-rose-200 rounded-xl p-6 shadow-sm flex flex-col justify-center">
              <span className="text-xs font-bold text-rose-600 mb-1">الرصيد المستحق (عليه)</span>
              <span className="text-3xl font-black text-rose-700 ">{selectedCustomer.balance.toLocaleString()} <span className="text-sm">د.ل</span></span>
            </div>
          </div>
              ...
              <table className="w-full text-right text-sm">
                <thead className="bg-slate-50 text-slate-600 font-semibold border-b border-slate-100 ">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-emerald-600">دائن (له)</th>
                    <th className="px-6 py-4 text-rose-600">مدين (عليه)</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 ">
                  {selectedCustomer.transactions.map((tx, idx) => (
                    <tr key={tx.id || idx}>
                      ...
                      <td className="px-6 py-4 text-emerald-600 font-bold">{tx.type === 'دائن' ? tx.amount.toLocaleString() : '-'}</td>
                      <td className="px-6 py-4 text-rose-600 font-bold">{tx.type === 'مدين' ? tx.amount.toLocaleString() : '-'}</td>
                    </tr>
                  ))}
                  {selectedCustomer.transactions.length === 0 && (
                    <tr>
                      <td colSpan={4} className="px-6 py-12 text-center text-slate-600">لا توجد حركات مالية مسجلة</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200/80 rounded-xl shadow-sm w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-slate-900 ">تسجيل دفعة سداد</h3>
            ...
            <form onSubmit={handleAddPayment} className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-2">المبلغ المدفوع (دائن)</label>
                <input 
                  type="number" 
                  required
                  min="0"
              ...
              <div>
                ...
                <input 
                  ...
                  placeholder="مثال: دفعة سداد نقدية"
                  className="w-full px-4 py-2.5 rounded-xl border border-slate-200/80 bg-slate-50 focus:outline-none focus:ring-1 focus:ring-slate-300 focus:border-slate-300 /20"
                />
              </div>
