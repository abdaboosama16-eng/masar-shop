    ...
    <div ref={containerRef} className="relative w-full max-w-md">
      ...
      <div className="relative">
        <input
          ...
          className="w-full glass-input rounded-xl pl-9 pr-10 py-2 text-xs font-medium bg-white/90 dark:bg-slate-800/90 border border-slate-200/90 dark:border-slate-700 shadow-xs focus:ring-2 focus:ring-emerald-500/20"
        />
      ...
      {isOpen && query.trim() !== '' && (
        <div className="absolute top-full mt-2 right-0 w-full md:w-[480px] bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-2xl shadow-2xl border border-slate-200/90 dark:border-slate-700 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
          
          <div className="p-3 bg-slate-50/80 dark:bg-slate-800/60 border-b border-slate-200/70 dark:border-slate-700/60 flex items-center justify-between text-xs text-slate-500">
            <span>نتائج البحث عن: <strong className="text-slate-900 dark:text-white">"{query}"</strong></span>
            <span className="text-[10px]">اضغط على أي نتيجة للانتقال المباشر</span>
          </div>

          <div className="max-h-[65vh] overflow-y-auto p-2 divide-y divide-slate-100 dark:divide-slate-800 space-y-2">
                ...
                <div className="space-y-1">
                  {results.orders.map(order => (
                    <button
                      ...
                      className="w-full text-right p-2.5 rounded-xl hover:bg-emerald-50/80 dark:hover:bg-slate-800 transition-colors flex items-center justify-between group cursor-pointer"
                    >
                      <div className="min-w-0 flex items-center gap-2.5">
                        <span className="font-mono text-xs font-black text-slate-900 dark:text-white bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-600 shrink-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[10px] text-slate-500 dark:text-slate-400 truncate max-w-[240px]">
                            {order.description}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <ChevronLeft size={14} className="text-slate-400 group-hover:text-emerald-600 group-hover:-translate-x-0.5 transition-all" />
                      </div>
                    </button>
                  ))}
                </div>
                ...
                <div className="space-y-1">
                  {results.clients.map(client => (
                    <button
                      ...
                      className="w-full text-right p-2.5 rounded-xl hover:bg-blue-50/80 dark:hover:bg-slate-800 transition-colors flex items-center justify-between group cursor-pointer"
                    >
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-lg bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300 flex items-center justify-center font-bold text-xs">
                      ...
                      <div className="flex items-center gap-2 text-xs">
                        ...
                        <ChevronLeft size={14} className="text-slate-400 group-hover:text-blue-600 group-hover:-translate-x-0.5 transition-all" />
                      </div>
                    </button>
                  ))}
                </div>
                ...
                <div className="space-y-1">
                  {results.materials.map(item => (
                    <button
                      ...
                      className="w-full text-right p-2.5 rounded-xl hover:bg-amber-50/80 dark:hover:bg-slate-800 transition-colors flex items-center justify-between group cursor-pointer"
                    >
                      <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <ChevronLeft size={14} className="text-slate-400 group-hover:text-amber-600 group-hover:-translate-x-0.5 transition-all" />
                      </div>
                    </button>
                  ))}
                </div>
